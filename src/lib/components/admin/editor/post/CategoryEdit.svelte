<script>
    ...
    let lastSavedCategories = JSON.parse(JSON.stringify(categories))
    ...
    async function pushChanges() {
        ...
        const {status, data} = await sendManageRequest('/post/updateCategories', {
            categories: categories.map(genre => genre.name),
            postId: postId
        })
    ...
    async function getAllCategories() {
        const {status, data} = await fetch(`/api/data/categories`).then(r => r.json())
    ...
    function addCategory(selected = '') {
        if (selected === '') return
        if (categories.find(genre => genre.name === selected)) return
        categories.push({name: selected})
    ...
    $: {
        // i want to die
        isUpToDate = arraysEqual(categories, lastSavedCategories)
    }
...
<div class="grid grid-cols-1 bg-stone-500 max-w-[22rem] m-1 text-white min-h-min bg-stone-500">
    <div class="ml-1 mr-1 mt-1">
        <p class="text-xl float-left">Categories</p>
        ...
        <div class="text-black">
            ...
            {:then genresAll}
                <Typeahead hideLabel
                           ...
                           let:result
                           limit={5}
                           on:select={({ detail }) => {addCategory(detail.selected)}}>
                    <strong>{@html result.string}</strong>

                    <svelte:fragment slot="no-results">
                        <p>
                            Could not find category "{value}",
                            <button on:click={()=>{addCategory(value)}} class="underline text-blue-600">Create it</button>
                        </p>
                    </svelte:fragment>
                </Typeahead>
            {/await}
                ...
                {#each categories as category}
                    <p class="p-1 bg-stone-700">
                        ...
                        <button class="w-1/6 float-right text-xl"
                                on:click={()=>{removeCategory(category.name)}}>🗑️
                        </button>
                    </p>
                {/each}
            {/key}
