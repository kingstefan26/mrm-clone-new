...
<div class="bg-stone-600 max-w-full">
    ...
    <div class="w-full h-min flex ">
        ...
        <h1 class="text-white text-xl font-bold w-5/6 text-center">
            Editing Chapter
        </h1>
        <button on:click={deleteChapter} class="bg-stone-500 hover:bg-stone-700 text-white font-bold px-4">
            Delete Chapter
        </button>
    </div>
    ...
    <div class="flex flex-wrap list">
        {#each data.chapter.assets as asset, index (asset.id)}
            <div
                ...
                <AssetPicker on:assetpicked={(event) => {changeAsset(index, asset, event)}} assetId="{asset.id}"/>
            </div>
        {/each}
        <div class="list-item">
            <p class="">
